...
?>
	<?php if (!isset($_SESSION['kuis_kalimat'])) {
		header("location: kuis-kalimat.php?n=1");
	}
	?>
	<html lang="en">
...
======================================================== -->
	...
	<body>
				...
				<div class="container">
					<ul class="list-group">
						...
						<?php if (isset($_SESSION['kuis_kalimat'])) {
							...
							$total_point = $_SESSION['kuis_kalimat'];
							if ($total_point >= $nilai) {
								echo '<li class="list-group-item"><strong>Selamat!</strong> Anda Berhasil Mengerjakan Kuis</li>';
							} else {
								echo '<li class="list-group-item"><strong>Anda belum berhasil!</strong> Anda perlu mengerjakan kuis lagi</li>';
							}
					...
					<a href="kuis-kalimat.php?n=1" class="btn btn-primary mt-3">Coba Lagi Kuis</a>
					<a href="../../halaman_belajar.php" class="btn btn-danger mt-3 ml-2">Keluar</a>
					<a href="../../kalimat.php" class="btn btn-info mt-3 ml-2">Next Materi</a>
				</div>
			</main>
		...
		$kuis_kalimat = $_SESSION['kuis_kalimat'];
		$id_user = $_SESSION['id_user'];
		$query = "SELECT kuis_kalimat FROM rombel_belajar WHERE id_user = '$id_user'";
		$result = mysqli_query($mysqli, $query);
		if ($result) {
			$row = mysqli_fetch_assoc($result);
			$existing_kuis_kalimat = $row['kuis_kalimat'];
			if ($kuis_kalimat > $existing_kuis_kalimat) {
				$query = "UPDATE rombel_belajar SET kuis_kalimat = '$kuis_kalimat' WHERE id_user = '$id_user'";
				$run = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
			}
		}
		?>
		<?php unset($_SESSION['kuis_kalimat']); ?>
