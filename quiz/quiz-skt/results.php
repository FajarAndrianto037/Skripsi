...
?>
	<?php if (!isset($_SESSION['kuis_skt'])) {
		header("location: kuis-skt.php?n=1");
	}
	?>
	<html lang="en">
...
======================================================== -->
	...
	<body>
		...
		<body>
			...
			<main>
				<div class="container">
					...
					<a href="kuis-skt.php?n=1" class="btn btn-primary mt-3">Coba Lagi Kuis</a>
					<a href="../../halaman_belajar.php" class="btn btn-danger mt-3 ml-2">Keluar</a>
					<a href="../../kata.php" class="btn btn-info mt-3 ml-2">Next Materi</a>
				</div>
			</main>
		...
		$kuis_skt = $_SESSION['kuis_skt'];
		$id_user = $_SESSION['id_user'];
		$query = "SELECT kuis_skt FROM rombel_belajar WHERE id_user = '$id_user'";
		$result = mysqli_query($mysqli, $query);
		if ($result) {
			$row = mysqli_fetch_assoc($result);
			$existing_kuis_skt = $row['kuis_skt'];
			if ($kuis_skt > $existing_kuis_skt) {
				$query = "UPDATE rombel_belajar SET kuis_skt = '$kuis_skt' WHERE id_user = '$id_user'";
				$run = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
			}
		}
		?>
		<?php unset($_SESSION['kuis_skt']); ?>
