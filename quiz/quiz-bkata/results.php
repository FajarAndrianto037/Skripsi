...
?>
	<?php if (!isset($_SESSION['kuis_bkata'])) {
		header("location: kuis-bkata.php?n=1");
	}
	?>
	<html lang="en">
...
======================================================== -->
	...
	<body>
				...
				<div class="container">
					<ul class="list-group">
						...
						<?php if (isset($_SESSION['kuis_bkata'])) {
							...
							$total_point = $_SESSION['kuis_bkata'];
					...
					<a href="kuis-bkata.php?n=1" class="btn btn-primary mt-3">Coba Lagi Kuis</a>
					<a href="../../halaman_belajar.php" class="btn btn-danger mt-3 ml-2">Keluar</a>
					<a href="../quiz-pemula1/halaman-pemula1.php" class="btn btn-info mt-3 ml-2">Next Materi</a>
				</div>
			</main>
		...
		$kuis_bkata = $_SESSION['kuis_bkata'];
		$id_user = $_SESSION['id_user'];
		$query = "SELECT kuis_bkata FROM rombel_belajar WHERE id_user = '$id_user'";
		$result = mysqli_query($mysqli, $query);
		if ($result) {
			$row = mysqli_fetch_assoc($result);
			$existing_kuis_bkata = $row['kuis_bkata'];
			if ($kuis_bkata > $existing_kuis_bkata) {
				$query = "UPDATE rombel_belajar SET kuis_bkata = '$kuis_bkata' WHERE id_user = '$id_user'";
				$run = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
			}
		}
		?>
		<?php unset($_SESSION['kuis_bkata']); ?>
