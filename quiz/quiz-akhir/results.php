...
?>
	<?php if (!isset($_SESSION['kuis_akhir'])) {
		header("location: kuis-akhir.php?n=1");
	}
	?>
	<html lang="en">
...
======================================================== -->
	...
	<body>
		...
		<body>
			...
			<main>
				<div class="container">
					...
					<a href="kuis-akhir.php?n=1" class="btn btn-primary mt-3">Coba Lagi Kuis</a>
		...
		$kuis_akhir = $_SESSION['kuis_akhir'];
		$id_user = $_SESSION['id_user'];
		$query = "SELECT kuis_akhir FROM rombel_belajar WHERE id_user = '$id_user'";
		$result = mysqli_query($mysqli, $query);
		if ($result) {
			$row = mysqli_fetch_assoc($result);
			$existing_kuis_akhir = $row['kuis_akhir'];
			if ($kuis_akhir > $existing_kuis_akhir) {
				$query = "UPDATE rombel_belajar SET kuis_akhir = '$kuis_akhir' WHERE id_user = '$id_user'";
				$run = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
			}
		}
		?>
		<?php unset($_SESSION['kuis_akhir']); ?>
