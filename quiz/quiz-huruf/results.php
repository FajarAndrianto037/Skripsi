...
?>
	<?php
	if (!isset($_SESSION['kuis_huruf'])) {
		header("location: kuis-huruf.php?n=1");
	}
...
======================================================== -->
	...
	<body>
		...
		<body>
			...
			<main>
				<div class="container">
					<ul class="list-group">
						<?php
						echo '<li class="list-group-item"><strong>Jumlah Minimal Nilai :</strong> ' . $nilai . '</li>';
						?>
						<?php if (isset($_SESSION['kuis_huruf'])) {
							if (!isset($_SESSION['total_benar'])) {
								// Jika belum memiliki nilai, atur nilai menjadi 0
								$_SESSION['total_benar'] = 0;
							}
							$total_point = $_SESSION['kuis_huruf'];
					...
					<a href="kuis-huruf.php?n=1" class="btn btn-primary mt-3">Coba Lagi Kuis</a>
					<a href="../../halaman_belajar.php" class="btn btn-danger mt-3 ml-2">Keluar</a>
					<a href="../../suku-kata.php" class="btn btn-info mt-3 ml-2">Next Materi</a>
				</div>
			</main>
		...
		$kuis_huruf = $_SESSION['kuis_huruf'];
		$id_user = $_SESSION['id_user'];
		$query = "SELECT kuis_huruf FROM rombel_belajar WHERE id_user = '$id_user'";
		$result = mysqli_query($mysqli, $query);
		if ($result) {
			$row = mysqli_fetch_assoc($result);
			$existing_kuis_huruf = $row['kuis_huruf'];
			if ($kuis_huruf > $existing_kuis_huruf) {
				$query = "UPDATE rombel_belajar SET kuis_huruf = '$kuis_huruf' WHERE id_user = '$id_user'";
				$run = mysqli_query($mysqli, $query) or die(mysqli_error($mysqli));
			}
		}
		?>
		<?php unset($_SESSION['kuis_huruf']); ?>
