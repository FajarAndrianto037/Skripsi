  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function showWarning(e) {
        if (qno < 5) {
          e.preventDefault();
          alert('Anda belum menyelesaikan semua soal. Silakan selesaikan kuis sebelum meninggalkan halaman ini.');
        }
      }

      // Add event listener to all links
      document.querySelectorAll('a').forEach(function(link) {
        link.addEventListener('click', showWarning);
      });

      // Add event listener to all forms (except the current form)
      document.querySelectorAll('form').forEach(function(form) {
        if (form.getAttribute('action') !== 'proses-huruf.php') {
          form.addEventListener('submit', showWarning);
        }
      });
      ...
      window.addEventListener('beforeunload', function(e) {
        if (qno < 5) {
          var confirmationMessage = 'Anda belum menyelesaikan semua soal. Apakah Anda yakin ingin meninggalkan halaman ini?';
    ...
    <main id="main">
      ...
      if (isset($_SESSION['rombel_id'])) {
        if (isset($_GET['n']) && is_numeric($_GET['n'])) {
          ...
          header('location: kuis-huruf.php?n=' . $_SESSION['quiz']);
        ...
        if (!empty($_SESSION['qid_list'])) {
          ...
          $query = "SELECT * FROM kuis_pembelajaran WHERE jenis_kuis = 'huruf-angka' and qid NOT IN ($qid_list_str) ORDER BY RAND() LIMIT 1";
        } else {
          $query = "SELECT * FROM kuis_pembelajaran WHERE jenis_kuis = 'huruf-angka' ORDER BY RAND() LIMIT 1";
        }
        ...
        if (mysqli_num_rows($run) > 0) {
          ...
          // Menghitung jumlah total soal berdasarkan jenis 'huruf-angka'
          $total_huruf_angka = "SELECT * FROM kuis_pembelajaran WHERE jenis_kuis = 'huruf-angka'";
          $run_huruf_angka = mysqli_query($mysqli, $total_huruf_angka) or die(mysqli_error($mysqli));
          $total_huruf_angka_qn = mysqli_num_rows($run_huruf_angka);
        } else {
      ...
      <section class="page-section" id="contact">
        <div class="container d-flex justify-content-center align-items-center vh-50" id="quiz">
          <div class="question p-5  w-100">
            <p>Question <?php echo $qno; ?> of 10</p>
            <b>
              <h4 class="question w-100"><?php echo $qno, '. ', $question; ?></h4>
            </b>
            <br>
            <div class="mb-4">
              <form method="post" action="proses-huruf.php">
                <div class="question">
