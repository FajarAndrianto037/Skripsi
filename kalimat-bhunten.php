    ...
    <div id="right-panel" class="">
        ...
        <div id="page-wrapper">
                ...
                <div style="height:430px;" class="col-lg-12">
                    <div class="presentation" style="    position: absolute;height: 100%;width: 100%;    left: 0px;    top: 0px;    display: block;    overflow: hidden;background: #ffffff; /* Mengatur warna latar belakang menjadi putih */">
                        ...
                        <div class="slides">
                            ...
                            $query = "SELECT `madura`, `indonesia` FROM `kalimat` WHERE `tingkatan` = 'engghi bhunten' LIMIT $per_page OFFSET $offset";
                            $result = mysqli_query($mysqli, $query);

                            ?>
                            <br><br><br>
                            <div class="buttons-container">
                                <a href="kalimat-iye.php"><button type="button" class="btn btn-lg btn-danger">Kalimat Enje' Iyâ</button></a>
                                <a href="kalimat-enten.php"><button type="button" class="btn btn-lg btn-danger">Kalimat Engghi Enten</button></a>
                                <a href="kalimat-bhunten.php"><button type="button" class="btn btn-lg btn-danger">Kalimat Engghi Bhunten</button></a>
                            </div>
                            <table class="table table-bordered table-hover table-striped custom-table" style="text-align:center; width: 75%;">
                                <thead>
                                    <tr>
                                        <th style="text-align:center">Kalimat</th>
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            // Process the input to split it into words
                                            $input_kata = proses_fsa($h['madura']);
                                            $words = split_sentence($input_kata);

                                            // Combine all individual onClick calls into a single string
                                            $onClickCalls = '';
                                            foreach ($words as $word) {
                                                $onClickCalls .= $word;
                                            }
                                            //var_dump($onClickCalls);
                                            ?>

                                            <!-- Display the whole sentence as a single button -->

                                            <td> <button type="button" class="btn btn-warning btn-lg" onClick=play("<?php echo $onClickCalls; ?>")><?php echo $h['madura']; ?></button>
                                            </td>
                                            <td>
                                                <?php
                                                // Check if the function exists before declaring it
                                                ...
                                                // Process the input to split it into words
                                                $input_kata = proses_fsa($h['madura']);
                                                $tampil = split_sentence($h['madura']);
                                                $words = split_sentence($input_kata);
                                                //var_dump(($tampil[0]));
                                                // Loop through each word and create a button
                                                $i = 0;
                                                foreach ($words as $word) {
                                                ?>
                                                    <button type="button" class="btn btn-warning btn-lg" onClick=play("<?php echo $word; ?>")><?php echo $tampil[$i]; ?></button>
                                                <?php
                                                    $i++;
                                                }
                                                ?>
                            ...
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-center">
                                    ...
                                    $total_query = "SELECT COUNT(*) as total FROM `kalimat` WHERE `tingkatan` = 'engghi bhunten'";
            ...
            <div align="center" class="col-lg-4">
                <a href="quiz/quiz-kalimat/halaman-kalimat.php" class="btn btn-lg btn-primary" role="button">Kuis Kalimat</a>
            </div>
        </div>
    </div>
    <!-- /.content -->
