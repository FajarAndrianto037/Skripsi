...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE valid_kalimat SET status = ? WHERE kalimat_id= ?";
...
$query = "SELECT * FROM valid_kalimat";
                ...
                <div class="orders">
                    <div class="row">
                        <div class="col-xl-12">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="box-title">Validasi Kalimat </h4>
                                    <br>
                                    <?php
                                    ...
                                    $total_rows_query = "SELECT COUNT(*) AS total FROM valid_kalimat";
                                    if ($search) {
                                    ...
                                    $query1 = "SELECT * FROM valid_kalimat";
                                    if ($search) {
                                        $query1 .= " WHERE madura LIKE '%" . mysqli_real_escape_string($mysqli, $search) . "%'";
                                    }
                                    $query1 .= " ORDER BY kalimat_id ASC LIMIT $offset, $limit";
                                    ...
                                    <table class="table table-striped table-bordered display">
                                        ...
                                        <tbody>
                                            ...
                                            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                                                $kalimat_id = $_POST['kalimat_id'];
                                                $status = $_POST['status'];

                                                // Perbarui status di database
                                                $query_update = "UPDATE `valid_kalimat` SET `status` = '$status' WHERE `kalimat_id` = '$kalimat_id'";
                                                mysqli_query($mysqli, $query_update);

                                                // Jika statusnya 'permintaan diterima', masukkan data ke tabel suku_kata
                                                if ($status == 'permintaan diterima') {
                                                    $query_valid = "SELECT * FROM `valid_kalimat` WHERE `kalimat_id` = '$kalimat_id'";
                                                    $result_valid = mysqli_query($mysqli, $query_valid);
                                                    if ($result_valid) {
                                                        $data_valid = mysqli_fetch_assoc($result_valid);
                                                        $madura = $data_valid['madura'];
                                                        $indonesia = $data_valid['indonesia'];
                                                        $keterangan = $data_valid['keterangan'];
                                                        $tingkatan = $data_valid['tingkatan'];
                                                        $query_insert = "INSERT INTO `kalimat` VALUES ('','$madura', '$indonesia', '$keterangan', '$tingkatan')";
                                            ...
                                            while ($data = mysqli_fetch_assoc($result)) { ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo $data['madura']; ?></td>
                                                    <td><?php echo $data['indonesia']; ?></td>
                                                    ...
                                                    <td>
                                                        ...
                                                        <form method="POST" action="" style="display:inline;">
                                                            <input type="hidden" name="kalimat_id" value="<?php echo $data['kalimat_id']; ?>">
