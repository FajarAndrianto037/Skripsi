...
if (isset($_POST['tambah'])) {
    $skt = mysqli_real_escape_string($mysqli, $_POST['skt']);

    // Cek apakah data sudah ada
    $check_query = "SELECT * FROM `suku_kata` WHERE `skt` = '$skt'";
    $check_result = mysqli_query($mysqli, $check_query);

    if (mysqli_num_rows($check_result) > 0) {
        echo "<script type='text/javascript'>
        alert('Gagal Ditambah, Data sudah ada di database.');
        document.location = 'tambah_suku_kata.php';
        </script>";
    } else {
        // Menyusun query SQL untuk insert
        $query = "INSERT INTO `suku_kata` VALUES ('', '$skt')";
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" id="skt" name="skt" class="form-control" id="" required="">
                                        </div>
                                        <button type="button" class="btn btn-warning" onClick="karakter_e()">
                                            &egrave;</button>
                                        <button type="button" class="btn btn-success" onClick="karakter_a()">
                                            &acirc;</button><br><br>
                                        <button type="submit" name="tambah" class="btn btn-primary">Submit</button>
                                        <script>
                                            function karakter_e() {
                                                teks = document.getElementById("skt").value
                                                document.getElementById("skt").value = teks + "è"
                                                document.getElementById("skt").focus()
                                                return false
                                            }

                                            function karakter_a() {
                                                teks = document.getElementById("skt").value
                                                document.getElementById("skt").value = teks + "â"
                                                document.getElementById("skt").focus()
                                                return false
                                            }
                                        </script>
