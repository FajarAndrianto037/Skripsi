            ...
            <div class="animated fadeIn">
                ...
                <div class="orders">
                        ...
                        <div class="col-xl-12">
                            <div class="card">
                                ...
                                if (isset($_POST['tambah'])) {
                                    ...
                                    $stmt = $mysqli->prepare("INSERT INTO `suku_kata` (skt) VALUES (?)");
                                    ...
                                    if (!empty($_FILES['file']['name'])) {
                                        ...
                                        $filename = preg_replace("/[^a-zA-Z0-9_-]/", "", pathinfo($file, PATHINFO_FILENAME));
                                        $extension = pathinfo($file, PATHINFO_EXTENSION);

                                        // Membuat nama file lengkap dengan ekstensi
                                        $cleaned_file = $filename . '.' . $extension;

                                        // Menentukan jalur file tujuan
                                        $target_file = "../audio/" . $cleaned_file;

                                        // Debugging: Memeriksa jalur file tujuan
                                        error_log("Target file path: " . $target_file);

                                        // Memeriksa apakah direktori tujuan dapat ditulis
                                        if (!is_writable('../audio/')) {
                                            error_log("Directory ../audio/ is not writable.");
                                            echo "<script type='text/javascript'>
                    alert('Direktori tujuan tidak dapat ditulisi.');
                  </script>";
                                            exit;
                                        }
                                        ...
                                        if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
                                            // Menjalankan pernyataan yang dipersiapkan hanya jika file berhasil diunggah
                                            if ($stmt->execute()) {
                                                echo "<script type='text/javascript'>
                        alert('Data berhasil ditambahkan dan file berhasil diunggah.');
                        document.location = 'suku_kata.php'; 
                      </script>";
                                            } else {
                                                echo "<script type='text/javascript'>
                        alert('Data gagal ditambah: " . $stmt->error . "');
                        document.location = 'index.php';
                      </script>";
                                            }
                                        } else {
                                            // Debugging: Menampilkan kesalahan jika pengunggahan file gagal
                                            $error = $_FILES["file"]["error"];
                                            error_log("File upload error: " . $error);
                                            echo "<script type='text/javascript'>
                    alert('File gagal diunggah. Error: " . $error . "');
                  </script>";
                                        }
                                    } else {
                                        // Menjalankan pernyataan yang dipersiapkan jika tidak ada file yang diunggah
                                        if ($stmt->execute()) {
                                            echo "<script type='text/javascript'>
                    ...
                    document.location = 'index.php';
                                ...
                                <div class="card-body">
                                    <h4 class="box-title">Tambah Audio </h4>
                                    <br>
                                    <form method="POST" action="" enctype="multipart/form-data">
                                        <div class="mb-3">
                                            <label for="exampleInputkode1" class="form-label">Suku Kata</label>
                                            <input type="text" name="skt" class="form-control" id="" required>
                                        </div>
                                        <div class="mb-3">
                                            <label for="exampleInputkode1" class="form-label">Audio</label>
